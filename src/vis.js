    ...
    var render = function(div) {
        ...
        render.update = function(data, circleScales, dataTypes) {
            ...
            for (i = 0; i < 5; i++) {
                ...
                .on("click", function(d) {
                    // render.update(data, updateScales(radius), dataTypes)
                    // svg.selectAll("g g").remove()
                    // var selected = d3.select(this.parentNode)[0]

                    //delete everything remake graph with only one datatype
                    var dataTypeToShow = this.__data__.data.type;
                    // d3.select("svg").remove();

                    for (var i = 0; i < dataTypes.length; i++) {
                        if (dataTypes[i].type = dataTypeToShow) {
                    ...
                    //       //d={data: Object, value: 0.8526315789473684, startAngle: 0.9259430979001495, endAngle: 6.283185307179586, padAngle: 0}
                    //       var interpolate = d3.interpolate(0, 2*Math.PI);
                    //       return function(t) {
                    //         // d.outerRadius(radius-(5*radius/5))
                    //         // d.innerRadius(radius-(5*radius/5)-radius/5);
